...
@export var androRun : AudioStreamPlayer2D
@export var hide : Node
@export var hideInfo : Node
@export var hideCooldown : Timer
@export var hidingAndro : Node
@export var hidingAndroGone : Node
@export var hidingWait : Timer
@export var jumpscareAndro : Node
@export var cerealAmountLabel : Label
@export var cerealEatenSprite : Node
@export var hidingAudio : AudioStreamPlayer2D
...
var chipLevel = CustomNightLevels.chipLevel;
var socksLevel = CustomNightLevels.socksLevel;
var skunkLevel = CustomNightLevels.skunkLevel;
var androLevel = CustomNightLevels.androLevel;
...
var androRunning = false;
var androSeen = false;

var timeSinceAndroSeen = 0;
var timeSinceAndroEscaped = 0;

var hiding = false;

var canHide = true;

var canStopHiding = false;
...
func save():
	...
	saveGame.store_line(JSON.stringify(7));
		
	saveGame.store_line(JSON.stringify(hardModeBeaten))
	saveGame.store_line(JSON.stringify(true))
	saveGame.store_line(JSON.stringify(true))
...
func androJumpscare():
	...
	jumpscareAndro.visible = true;
	
	androRun.play();
	
	await get_tree().create_timer(2).timeout;
	
	androRun.set_volume_db(-100);
...
func _process(_delta):
	...
	if Input.is_action_just_pressed("Hide"):
		if androSeen == true && canHide == true && cameraUp == false:
			if hiding == false:
				androRun.volume_db = -80;
				...
				hidingAndro.visible = true;
				hidingAudio.playing = true;
				
				hideCooldown.start();
				hidingWait.start();
			elif hiding == true:
				if canStopHiding == true:
					hiding = false;

					hidingAndro.visible = false;
					hidingAndroGone.visible = false;

					androSeen = false;
					
					hidingAudio.playing = false;
					
					hideCooldown.start();
				elif canStopHiding == false:
					androJumpscare();
	if camera == 0:
		...
		if androSeen == false:
			...
			elif androProgress == 1:
				cameraView.set_texture(androStageTwo);
			elif androProgress == 2:
				cameraView.set_texture(androStageThree);
			elif androProgress == 3:
				cameraView.set_texture(androStageFour);
			elif androProgress == 4:
				cameraView.set_texture(androEmpty);
			
				androRun.volume_db = 15;
	...
	if mouseY > 951:
		...
		if canFlipCamera == true && inJumpscare == false && makingCereal == false && fadedIn == true && hiding == false:
			cameraFlip.play();
	...
	if mouseX > 1805:
		if canMakeCereal == true && inJumpscare == false && cameraUp == false && fadedIn == true && hiding == false:
			...
			elif makingCereal == false && !cerealEaten2:
...
func _on_timer_timeout():
	...
	if androSeen && !hiding:
		timeSinceAndroSeen += 1;
	
	if androProgress == 4 && androSeen == false:
		timeSinceAndroEscaped += 1;
	
	if timeSinceAndroSeen >= 5:
		androJumpscare();
		
	if timeSinceAndroEscaped >= 30:
		androJumpscare();
...
func _on_timer_socks_timeout():
	...
	if socksRandom < socksLevel || socksRandom == socksLevel || socksGodMode == true:
		if socksLazy == false:
			if socksCamera == 1:
				socksCamera = 2;
...
func _on_bowl_area_area_entered(area):
	if area.name == "cerealArea" && makingCereal == true && waiting == false:
		...
		cereal.position = Vector2(1055, 534);
		cereal.visible = true;
...
func _on_andro_timer_timeout():
	...
	if androRandom < androLevel || androRandom == androLevel || androGodMode == true && androLazy != true:
		if androProgress == 0:
			androProgress = 1;
		elif androProgress == 1:
			androProgress = 2;
...
func _on_hide_cooldown_timeout():
	canHide = true;

func _on_hiding_wait_timeout():
	hidingAndro.visible = false;
	hidingAndroGone.visible = true;
	canStopHiding = true;

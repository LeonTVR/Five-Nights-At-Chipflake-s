...
func _on_milk_button_button_up():
	if cerealDragging == false && bowlState != 2 && bowlState != 3 && makingCereal == true && hasCereal == false  && cerealAmount != 0:
		milkDragging = true;
...
func _on_bowl_area_area_entered(area):
	...
	if area.name == "cerealArea" && makingCereal == true && waiting == false:
		...
		print("It is better to cum in the sink than to sink in the cum - A wise man");
		...
		if bowlState == 1:
			...
			print("WHY WOULD YOU FUCKING DARE PUT MILK FIRST?!")
...
func _on_shoot_glock_button_up():
	...
	if cameraButtons.camera == 7 && waiting == false:
		socksLazy = true;
		...
		socksLazy = false;
	elif cameraButtons.camera == 5 && waiting == false:
		await get_tree().create_timer(1).timeout;
